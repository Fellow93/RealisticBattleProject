...
namespace RBM
{
    internal class CustomBattlePatches
    {
        ...
        private static string ShowSaveDialog()
        {
            ...
            string filter = new TextObject("{=RBM_CB_FILTER}XML Preset").ToString() + "|*.xml";
        ...
        private static void SavePresetToFile()
        {
            try
            {
                ...
                TextObject saved = new TextObject("{=RBM_CB_PRESET_SAVED}Preset saved: '{NAME}'.");
                saved.SetTextVariable("NAME", Path.GetFileName(path));
                InformationManager.DisplayMessage(new InformationMessage(saved.ToString()));
            }
            catch (Exception) { }
        }
        ...
        private static void LoadPresetFromFile()
        {
            try
            {
                ...
                if (preset != null)
                {
                    ...
                    TextObject loaded = new TextObject("{=RBM_CB_PRESET_LOADED}Preset loaded: '{NAME}'.");
                    loaded.SetTextVariable("NAME", Path.GetFileName(path));
                    InformationManager.DisplayMessage(new InformationMessage(loaded.ToString()));
                }
            }
            catch (Exception) { }
        }
