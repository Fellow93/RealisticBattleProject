...
namespace RBMCampaign
{
    ...
    public class SpoilsDrunkNotificationItemVM : SettlementNotificationItemBaseVM, IRBMNameplateNotification
    {
        ...
        // Turns of phrase for a stack drinking its surplus away. Each is a whole-sentence template: the figures ride in as text
        // variables, so a translation can put them wherever its own grammar wants them.
            ...
            : base(onRemove, createdTick)
        {
            ...
            base.Text = new TextObject(Flavors[MBRandom.RandomInt(Flavors.Length)])
                .SetTextVariable("AMT", amount).ToString();
    ...
    public class TroopFoodNotificationItemVM : SettlementNotificationItemBaseVM, IRBMNameplateNotification
    {
        ...
        // Turns of phrase for the player's party provisioning itself. Each is a whole-sentence template: the figures ride in as text
        // variables, so a translation can put them wherever its own grammar wants them.
        private static readonly string[] Flavors =
        {
            "{=RBM_FOOD_BUY_01}buys {LIST} — {AMT} denars",
            "{=RBM_FOOD_BUY_02}lays in {LIST} for {AMT} denars",
            "{=RBM_FOOD_BUY_03}provisions {LIST} — {AMT} denars",
            "{=RBM_FOOD_BUY_04}stocks up: {LIST} ({AMT} denars)",
            "{=RBM_FOOD_BUY_05}fills the packs — {LIST} for {AMT} denars"
        };
    ...
    public class TroopLuxuryNotificationItemVM : SettlementNotificationItemBaseVM, IRBMNameplateNotification
    {
        ...
        // Turns of phrase for a stack buying itself a keepsake. Each is a whole-sentence template: the figures ride in as text
        // variables, so a translation can put them wherever its own grammar wants them.
        private static readonly string[] Flavors =
        {
            "{=RBM_LUXURY_BUY_01}indulge in {LIST} — {AMT} denars",
            "{=RBM_LUXURY_BUY_02}treat themselves to {LIST} for {AMT} denars",
            "{=RBM_LUXURY_BUY_03}splash out on {LIST} — {AMT} denars",
            "{=RBM_LUXURY_BUY_04}fancy {LIST} ({AMT} denars)",
            "{=RBM_LUXURY_BUY_05}buy themselves {LIST} for {AMT} denars"
        };
