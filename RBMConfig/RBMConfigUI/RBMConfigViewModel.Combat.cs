...
namespace RBMConfig
{
    internal partial class RBMConfigViewModel
    {
        ...
        {
            get { return new TextObject("{=RBM_CFG_019}Frontline System").ToString(); }
        }

        [DataSourceProperty]
        public BasicTooltipViewModel FrontlineEnabledHint { get; } = Hint("{=RBM_CFG_020}Per-soldier jostling inside a charging infantry or archer line: each man decides every couple of seconds whether to press in, step back, close on a neighbour, slide around a flank or hold his rank. Off leaves melee lines on RBM's plain charge. Cavalry/archer free-charge rules and unit facing are unaffected either way. Default on.");
        ...
        {
            get { return new TextObject("{=RBM_CFG_021}Frontline Min Formation Size").ToString(); }
        }

        [DataSourceProperty]
        public BasicTooltipViewModel FrontlineMinFormationSizeHint { get; } = Hint("{=RBM_CFG_022}Formations smaller than this (undetached men) skip the frontline system entirely and charge normally -- there is no rank to hold in a handful of men. Default 25.");
        ...
        {
            get { return new TextObject("{=RBM_CFG_023}Frontline Decision Hold").ToString(); }
        }

        [DataSourceProperty]
        public BasicTooltipViewModel FrontlineDecisionTimerMaxHint { get; } = Hint("{=RBM_CFG_024}Once a man picks a move he sticks with it for a random 0 to this many seconds before reconsidering. Higher is steadier and cheaper; lower makes the line twitchier. Default 2.00.");
        ...
        {
            get { return new TextObject("{=RBM_CFG_025}Frontline Attack Weight").ToString(); }
        }

        [DataSourceProperty]
        public BasicTooltipViewModel FrontlineAttackWeightHint { get; } = Hint("{=RBM_CFG_026}Multiplier on the 'press forward at my target' score. Above 1 makes lines more aggressive and thinner; below 1 makes them hang back. Default 1.00.");
        ...
        {
            get { return new TextObject("{=RBM_CFG_027}Frontline Back Step Weight").ToString(); }
        }

        [DataSourceProperty]
        public BasicTooltipViewModel FrontlineBackStepWeightHint { get; } = Hint("{=RBM_CFG_028}Multiplier on the 'give ground and let a fresh man through' score. Above 1 gives more rotation out of the front rank. Default 1.00.");
        ...
        {
            get { return new TextObject("{=RBM_CFG_029}Frontline Close Ranks Weight").ToString(); }
        }

        [DataSourceProperty]
        public BasicTooltipViewModel FrontlineFindAllyWeightHint { get; } = Hint("{=RBM_CFG_030}Multiplier on the 'close the gap to my nearest neighbour' score. Above 1 makes lines huddle tighter and shield walls hold better; below 1 lets them spread. Default 1.00.");
        ...
        {
            get { return new TextObject("{=RBM_CFG_031}Frontline Sidestep Weight").ToString(); }
        }

        [DataSourceProperty]
        public BasicTooltipViewModel FrontlineFlankWeightHint { get; } = Hint("{=RBM_CFG_032}Multiplier on both the left and right 'slide sideways past the man in front' scores. Above 1 makes lines spread wide around a stalled front rank. Default 1.00.");
